...
async function kvGet(): Promise<WebStory[]> {
  ...
  return (await redis.get<WebStory[]>(KV_KEY)) || []
}

async function kvSet(stories: WebStory[]): Promise<void> {
  const redis = await getRedis()
  await redis.set(KV_KEY, stories)
}
...
export function addStory(story: WebStory): void {
  ...
  else { const all = fileGet(); if (all.find(s => s.slug === story.slug)) return; all.unshift(story); fileSet(all) }
}
