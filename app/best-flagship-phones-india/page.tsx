...
export default async function BestFlagshipPhonesPage() {
  const { month, year } = currentMonthYear()
  const articles = await getPillarArticles(['flagship', 'premium', 'ultra', 'pro max', 'high end', 'best android', 'luxury phone'], [], 15)
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          <section className="mb-8">
            <h2 className="font-playfair text-xl font-bold text-ink mb-4 pb-2 border-b border-border">
              Is a Flagship Phone Worth It in India?
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">7-year updates economics</p>
                <p className="font-sans text-xs text-muted leading-relaxed">A Rs 1 lakh phone with 7-year updates costs Rs 14000 per year. A Rs 25K phone replaced every 2 years costs Rs 12500 per year — but with worse daily experience.</p>
              </div>
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">Camera as a business tool</p>
                <p className="font-sans text-xs text-muted leading-relaxed">Flagship cameras replace a Rs 50K DSLR for content creators. This is a genuine business investment for many Indian buyers.</p>
              </div>
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">Resale value advantage</p>
                <p className="font-sans text-xs text-muted leading-relaxed">iPhone retains 50-60% value after 2 years. Most Chinese flagships retain only 25-35%. Factor this into the total cost.</p>
              </div>
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">Import duty reality in India</p>
                <p className="font-sans text-xs text-muted leading-relaxed">Flagships cost 30-40% more in India than US pricing due to import duties. The math still works for 3+ year ownership.</p>
              </div>
            </div>
          </section>
