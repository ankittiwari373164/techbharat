...
function getInternalLinkMap(articleBrand?: string): [RegExp, string, string][] {
  ...
  // Brand-specific pillar links injected first — take highest priority
  if (brand === 'samsung') {
    brandPillarLinks.push([
      /\b(Samsung|Galaxy|S24|Z Fold)\b/gi,
      '/best-samsung-phones-india',
      'Best Samsung Smartphones India 2026',
    ])
  } else if (brand === 'apple' || brand === 'iphone') {
    brandPillarLinks.push([
      /\b(Apple|iPhone|iOS|MacBook)\b/gi,
      '/best-apple-iphone-india',
      'Best iPhones in India 2026',
    ])
  } else if (brand === 'oneplus') {
    brandPillarLinks.push([
      /\b(OnePlus|OxygenOS|Nord)\b/gi,
      '/best-oneplus-phones-india',
      'Best OnePlus Phones India 2026',
    ])
  }
  ...
  return [
    ...
    // ── Brand news links ───────────────────────────────────────────────────
    [/\b(Xiaomi|Redmi Note|Mi)\b/gi,       '/mobile-news?brand=Xiaomi',   'Latest Xiaomi news'],
    [/\b(Realme|Realme GT)\b/gi,           '/mobile-news?brand=Realme',   'Latest Realme news'],
    [/\b(Vivo|Vivo X)\b/gi,                '/mobile-news?brand=Vivo',     'Latest Vivo news'],
    [/\b(OPPO|Reno|Find X)\b/gi,           '/mobile-news?brand=OPPO',     'Latest OPPO news'],
    [/\b(iQOO|iQOO Neo)\b/gi,              '/mobile-news?brand=iQOO',     'Latest iQOO news'],
    [/\b(Poco|Poco X|Poco F)\b/gi,         '/mobile-news?brand=Poco',     'Latest Poco news'],
    [/\b(Redmi)\b/gi,                      '/mobile-news?brand=Xiaomi',   'Latest Redmi news'],
    [/\b(Motorola|Moto Edge|Moto G)\b/gi,  '/mobile-news?brand=Motorola', 'Latest Motorola news'],
    [/\b(Nothing Phone|Nothing)\b/gi,      '/mobile-news?brand=Nothing',  'Latest Nothing Phone news'],
    [/\b(Google Pixel|Pixel)\b/gi,         '/mobile-news?brand=Google',   'Latest Google Pixel news'],
    [/\b(Honor|Honor Magic)\b/gi,          '/mobile-news?brand=Honor',    'Latest Honor news'],

    // ── Pillar page topic links ────────────────────────────────────────────
    [
      /\b(buying guide|how to (buy|choose) a (smartphone|phone)|which phone (to buy|should i get))\b/gi,
      '/smartphone-buying-guide-india',
      'Smartphone Buying Guide India 2026',
    ],
    [
      /\b(best (smartphones?|phones?) (in|for) India)\b/gi,
      '/best-smartphones-india',
      'Best Smartphones India 2026',
    ],
    [
      /\b(best camera phone|top camera phone|photography phone|camera focused)\b/gi,
      '/best-camera-phones-india',
      'Best Camera Phones India 2026',
    ],
    [
      /\b(gaming phone|BGMI|Genshin Impact phone|best phone for gaming)\b/gi,
      '/best-gaming-phones-india',
      'Best Gaming Phones India 2026',
    ],
    [
      /\b(battery backup|long battery|all-day battery|best battery phone)\b/gi,
      '/best-battery-backup-phones-india',
      'Best Battery Phones India 2026',
    ],
    [
      /\b(best 5G phone|5G smartphone|5G available|top 5G device)\b/gi,
      '/best-5g-phones-india',
      'Best 5G Phones India 2026',
    ],
    [
      /\b(flagship phone|premium smartphone|best flagship|flagship model)\b/gi,
      '/best-flagship-phones-india',
      'Best Flagship Phones India 2026',
    ],
    [
      /\b(budget phone under|best phone under ₹|affordable phone|low cost phone)\b/gi,
      '/best-budget-phones-india',
      'Best Budget Phones India 2026',
    ],
    [
      /\b(phone comparison|compare phones?|head-to-head|versus)\b/gi,
      '/phone-comparison-guide-india',
      'Phone Comparison Guide India',
    ],
    [
      /\b(battery health|battery degradation|charging habit|extend battery life)\b/gi,
      '/android-battery-health-guide',
      'Android Battery Health Guide',
    ],

    // ── Content section links ──────────────────────────────────────────────
    [/\b(phone review|hands-on|first look|tested)\b/gi, '/reviews',     'Phone reviews India'],
    [/\b(compare|head-to-head|versus|comparison)\b/gi, '/compare',     'Compare phones India'],
    [/\b(deal|discount|offer|Flipkart|Amazon India)\b/gi, '/mobile-news', 'Best phone deals India'],
    [/\b(Jio|Airtel|5G launch|network coverage)\b/gi,   '/mobile-news', 'Latest 5G news India'],
...
function addInternalLinks(html: string, _currentSlug: string, articleBrand?: string): string {
  ...
  return parts
    .map(part => {
      ...
      // Inside an existing anchor — don't nest links
      ...
      for (const [regex, url, title] of getInternalLinkMap(articleBrand)) {
        ...
        const newText = text.replace(regex, match => {
          ...
          linked.add(url)
          return `<a href="${url}" title="${title}" class="internal-link text-[#1a3a5c] hover:text-[#d4220a] font-semibold underline decoration-dotted underline-offset-2 transition-colors">${match}</a>`
        })
...
const BRAND_DETECT_MAP: [RegExp, string][] = [
  ...
  [/\b(Google Pixel|Pixel)\b/i, 'Google Pixel'],
  [/\bHonor\b/i,             'Honor'],
]
...
export default function ArticleClient({ article, similar, slug }: ArticleClientProps) {
  ...
  const safeImages = (Array.isArray(liveArticle.images) ? liveArticle.images : []).filter(
    (img: string) => img && !img.startsWith('/phone-images/') && !img.includes('picsum')
  )
  ...
  // Resolve brand — if generic 'Mobile', detect from title
  const resolvedBrand = (() => {
  ...
  // Format publish date with UTC to avoid server/client hydration mismatch
  ...
  items={[
    { label: 'Home', href: '/' },
    { label: liveArticle.category || 'News', href: `/${liveArticle.type}` },
    { label: liveArticle.title, href: `/${slug}` }
  ]}
/>
            ...
            <div className="flex items-center gap-2 mb-3">
              <span
                className={`${TYPE_COLORS[liveArticle.type] || 'bg-[#1a3a5c]'} text-white font-sans text-[10px] font-bold px-2.5 py-1 uppercase tracking-widest`}
              >
            ...
            <div className="flex flex-wrap items-center gap-3 border-b border-border pb-4 mb-5">
              ...
              <div className="ml-auto flex items-center gap-2">
                <a
                  ...
                  className="font-sans text-xs bg-[#25D366] text-white px-2.5 py-1 hover:opacity-80 transition-opacity"
                >
                  Share
                </a>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="font-sans text-xs bg-[#2AABEE] text-white px-2.5 py-1 hover:opacity-80 transition-opacity"
                >
                  Telegram
                </a>
              </div>
            ...
            <div className="relative mb-5 overflow-hidden" style={{ paddingBottom: '56.25%' }}>
              <img
                ...
                onError={e => {
                  ...
                  if (t.src !== 'https://thetechbharat.com/og-image.jpg')
                    t.src = 'https://thetechbharat.com/og-image.jpg'
                }}
              />
            </div>
            ...
            <div className="article-body-full">
              ...
              {safeImages[1] && (
                <div className="relative my-6 overflow-hidden" style={{ paddingBottom: '50%' }}>
                  <img
                    src={safeImages[1]}
                    alt={`${liveArticle.title} — detailed view`}
                    width={800}
                    height={450}
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
                    loading="lazy"
              ...
              {/* Full Content — h1 tags demoted to h2 (page already has one H1) */}
              <div
              ...
              {safeImages[2] && (
                <div className="relative my-6 overflow-hidden" style={{ paddingBottom: '50%' }}>
                  <img
                    src={safeImages[2]}
                    alt={`${liveArticle.title} — additional image`}
                    width={800}
                    height={450}
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
                    loading="lazy"
              ...
              {safeTags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-6 pt-4 border-t border-border">
                  {safeTags.map(tag => (
                    <span
                      key={tag}
                      className="font-sans text-xs bg-gray-100 text-muted px-3 py-1 border border-border"
                    >
                      #{tag}
                    </span>
                  ))}
            ...
            <div className="mt-4 p-4 bg-[#f8f4ef] border-l-4 border-[#d4220a]">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {resolvedBrand && resolvedBrand !== 'Mobile' && (
                  <a
                    href={`/mobile-news?brand=${resolvedBrand}`}
                    className="font-sans text-xs text-ink border border-border bg-white px-3 py-1.5 hover:border-[#d4220a] hover:text-[#d4220a] transition-colors"
                  >
                    🔍 More {resolvedBrand} News
                  </a>
                )}
              </div>
            </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-0.5 mb-2">
                            {Array.from({ length: 5 }).map((_, si) => (
                              <svg
                                key={si}
                                className={`w-3.5 h-3.5 ${si < review.rating ? 'text-amber-400' : 'text-gray-200'}`}
                                fill="currentColor"
                                viewBox="0 0 20 20"
                              >
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {[1, 2, 3, 4, 5].map(star => (
                      <button key={star} onClick={() => setReviewRating(star)} type="button">
                        <svg
                          className={`w-5 h-5 ${star <= reviewRating ? 'text-amber-400' : 'text-gray-200'}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          ...
          <aside className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white border border-border p-5">
              ...
              <p className="font-sans text-xs text-muted leading-relaxed">
                Vijay has reviewed 300+ devices and covered the Indian smartphone market for 11 years.
                Founder of The Tech Bharat, based in New Delhi.
              </p>
              <Link
                href="/author"
                className="font-sans text-xs font-semibold text-[#d4220a] mt-2 inline-block hover:underline"
              >
                View Author Profile →
              </Link>
            </div>
            ...
            <div className="bg-[#1a3a5c] p-5 text-white">
              ...
              <div className="space-y-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block bg-[#2AABEE] text-white font-sans text-xs font-semibold text-center py-2 hover:opacity-90 transition-opacity"
                >
                  Join Telegram →
                </a>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block bg-[#25D366] text-white font-sans text-xs font-semibold text-center py-2 hover:opacity-90 transition-opacity"
                >
                  Join WhatsApp →
                </a>
              </div>
