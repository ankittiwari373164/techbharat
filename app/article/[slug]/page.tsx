...
export default function ArticlePage() {
  ...
  // Robust slug: from useParams (rewrite) or window.location (direct clean URL)
  ...
  const safeImages   = Array.isArray(article.images)        ? article.images        : []
  const safeBullets  = Array.isArray(article.bullets)       ? article.bullets       : []
  const safeTags     = Array.isArray(article.tags)          ? article.tags          : []
  const safeReviews  = Array.isArray(article.reviews)       ? article.reviews       : []
  const safeQSBullets = Array.isArray(article.quickSummary?.bullets) ? article.quickSummary.bullets : []
  const safeQSBrand   = article.quickSummary?.brand || article.brand || ''
  const safeQSDate    = article.quickSummary?.date  || ''
          ...
          <article className="lg:col-span-2">
            ...
            <div className="flex flex-wrap items-center gap-3 border-b border-border pb-4 mb-5">
              ...
              <div className="ml-auto flex items-center gap-2">
                <a
                  href={`https://wa.me/?text=${encodeURIComponent(article.title + ' ' + (process.env.NEXT_PUBLIC_SITE_URL || 'https://thetechbharat.com') + '/' + article.slug)}`}
                  target="_blank" rel="noopener noreferrer"
                  className="font-sans text-xs bg-[#25D366] text-white px-2.5 py-1 hover:opacity-80"
                >Share</a>
                <a
                  href={`https://t.me/share/url?url=${encodeURIComponent((process.env.NEXT_PUBLIC_SITE_URL || 'https://thetechbharat.com') + '/' + article.slug)}&text=${encodeURIComponent(article.title)}`}
                  target="_blank" rel="noopener noreferrer"
              ...
              <div className="animate-fade-in-up">
                ...
                {safeImages[1] && (
                  <div className="relative my-6 overflow-hidden" style={{ paddingBottom: '50%' }}>
                    <img src={safeImages[1]} alt={`${article.title} detail`} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} onError={(e)=>{(e.target as HTMLImageElement).src="https://picsum.photos/seed/tb2/800/600"}} />
                  </div>
                )}
                ...
                {safeImages[2] && (
                  <div className="relative my-6 overflow-hidden" style={{ paddingBottom: '50%' }}>
                    <img src={safeImages[2]} alt={`${article.title} image 3`} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} onError={(e)=>{(e.target as HTMLImageElement).src="https://picsum.photos/seed/tb3/800/600"}} />
                  </div>
                )}
