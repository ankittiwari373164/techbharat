...
export default async function BestGamingPhonesPage() {
  const { month, year } = currentMonthYear()
  const articles = await getPillarArticles(['gaming', 'BGMI', 'fps', 'refresh rate', 'performance', 'processor', 'cooling', 'benchmark', 'game'], [], 15)
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          <section className="mb-8">
            <h2 className="font-playfair text-xl font-bold text-ink mb-4 pb-2 border-b border-border">
              What Gaming Phones Actually Need in India
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">Sustained performance test</p>
                <p className="font-sans text-xs text-muted leading-relaxed">Holding 90fps for 45 minutes without throttling is the real BGMI benchmark. Peak AnTuTu scores are meaningless for real play.</p>
              </div>
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">Vapour chamber cooling</p>
                <p className="font-sans text-xs text-muted leading-relaxed">Separates playable from throttled in 40 degree Indian summer. iQOO and ASUS ROG have the best thermal systems.</p>
              </div>
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">120Hz display minimum</p>
                <p className="font-sans text-xs text-muted leading-relaxed">120Hz is the minimum for competitive BGMI. Touch sampling rate above 240Hz improves aiming precision noticeably.</p>
              </div>
              <div className="bg-white border border-border p-4">
                <p className="font-sans text-xs font-bold text-ink mb-1">Shoulder triggers</p>
                <p className="font-sans text-xs text-muted leading-relaxed">Physical or capacitive shoulder triggers give competitive advantage in BGMI gunfights — rare below Rs 30K.</p>
              </div>
            </div>
          </section>
